 ...
 * Augmented Red-Black Interval<T> Tree Implementation
 *
 ...
 * - Interval<T> augmentation with maxEndValue for efficient overlap detection
 ...
 */
export class RedBlackNode<T extends {} | null> {
  interval: Run<T>; // The interval data stored in this node
  color: NodeColor; // Red or Black color for RB tree balancing
  parent: RedBlackNode<T> | null; // Pointer to parent node (null for root)
  left: RedBlackNode<T> | null; // Pointer to left child
  right: RedBlackNode<T> | null; // Pointer to right child
   ...
   */
  constructor(interval: Run<T>, color: NodeColor = NodeColor.RED) {
 ...
 */
export class RedBlackIntervalTree<T extends {} | null> {
  private root: RedBlackNode<T> | null;
  private size: number;
   ...
   */
  private updateAncestorsMaxEndValue(node: RedBlackNode<T> | null): void {
  ...
  /**
   * Updates the maxEndValue augmentation for a node and all its ancestors.
   * Call this after directly modifying an interval's start or end position.
   *
   * @param node - The node whose interval was modified
   */
  updateNodeAndAncestors(node: RedBlackNode<T>): void {
    this.updateAncestorsMaxEndValue(node);
  }
   ...
   */
  insert(interval: Run<T>): void {
    const newNode = new RedBlackNode<T>(interval);
   ...
   */
  private insertFixup(node: RedBlackNode<T>): void {
   ...
   */
  private findNode(interval: Run<T>): RedBlackNode<T> | null {
    ...
    while (current !== null) {
      if (interval.start < current.interval.start) {
        ...
        if (interval.end === current.interval.end && interval.data === current.interval.data) {
          return current; // Exact match found!
        } else if (interval.end < current.interval.end || interval.end === current.interval.end) {
   ...
   * - Interval<T> tree augmentation (maxEndValue updates)
   *
   * Key variables:
   ...
   */
  public deleteNode(nodeToDelete: RedBlackNode<T>): void {
    ...
    let replacementNode: RedBlackNode<T> | null;
   ...
   */
  private minimum(node: RedBlackNode<T>): RedBlackNode<T> {
   ...
   * This is a convenience method that wraps findOverlappingNodes()
   */
  findOverlapping(queryInterval: Run<T>): Run<T>[] {
    return this.findOverlappingNodes(queryInterval).map((node) => node.interval);
  }

  /**
   * Finds all nodes whose intervals overlap with the given interval
   * This is more efficient when you need to perform operations on the nodes themselves
   * (e.g., deletion) as it avoids re-traversing the tree with findNode()
   */
  findOverlappingNodes(queryInterval: Run<T>): RedBlackNode<T>[] {
    const result: RedBlackNode<T>[] = [];
    this.findOverlappingNodesHelper(this.root, queryInterval, result);
    return result;
  }

  /**
   * Helper method for finding overlapping nodes
   */
  private findOverlappingNodesHelper(
    node: RedBlackNode<T> | null,
    queryInterval: Run<T>,
    result: RedBlackNode<T>[],
  ): void {
    if (node === null) {
      return;
    }
    ...
    if (node.left !== null && node.left.maxEndValue >= queryInterval.start) {
      this.findOverlappingNodesHelper(node.left, queryInterval, result);
    }

    // Check right subtree if it might contain overlapping intervals
    if (node.right !== null && node.interval.start <= queryInterval.end) {
      this.findOverlappingNodesHelper(node.right, queryInterval, result);
    }
  }
   ...
   */
  private intervalsOverlap(interval1: Run<T>, interval2: Run<T>): boolean {
    return interval1.start <= interval2.end && interval2.start <= interval1.end;
  }

  /**
   * Finds all intervals that contain the given point
   * This is a convenience method that wraps findContainingNodes()
   */
  findContaining(point: number): Run<T>[] {
    return this.findContainingNodes(point).map((node) => node.interval);
  }

  /**
   * Finds all nodes whose intervals contain the given point
   * This is more efficient when you need to perform operations on the nodes themselves
   * (e.g., deletion) as it avoids re-traversing the tree with findNode()
   */
  findContainingNodes(point: number): RedBlackNode<T>[] {
    const result: RedBlackNode<T>[] = [];
    this.findContainingNodesHelper(this.root, point, result);
    return result;
  }

  /**
   * Helper method for finding nodes containing a point
   */
  private findContainingNodesHelper(
    node: RedBlackNode<T> | null,
    point: number,
    result: RedBlackNode<T>[],
  ): void {
    ...
    if (node.left !== null && node.left.maxEndValue >= point) {
      this.findContainingNodesHelper(node.left, point, result);
    }

    // Check right subtree if it might contain intervals that include the point
    if (node.right !== null && node.interval.start <= point) {
      this.findContainingNodesHelper(node.right, point, result);
    }
  }

  /**
   * Finds all intervals in the given range [start, end]
   * This is a convenience method that wraps findInRangeNodes()
   */
  findInRange(start: number, end: number): Run<T>[] {
    return this.findInRangeNodes(start, end).map((node) => node.interval);
  }

  /**
   * Finds all nodes whose intervals are completely contained in the given range [start, end]
   * This is more efficient when you need to perform operations on the nodes themselves
   * (e.g., deletion) as it avoids re-traversing the tree with findNode()
   */
  findInRangeNodes(start: number, end: number): RedBlackNode<T>[] {
    const result: RedBlackNode<T>[] = [];
    this.findInRangeNodesHelper(this.root, start, end, result);
    return result;
  }

  /**
   * Helper method for finding nodes in a range
   */
  private findInRangeNodesHelper(
    node: RedBlackNode<T> | null,
    start: number,
    end: number,
    result: RedBlackNode<T>[],
    ...
    if (node.left !== null && node.left.maxEndValue >= start) {
      this.findInRangeNodesHelper(node.left, start, end, result);
    }

    // Check right subtree
    if (node.right !== null && node.interval.start <= end) {
      this.findInRangeNodesHelper(node.right, start, end, result);
    }
  }
   ...
   */
  findAnyOverlapping(queryInterval: Run<T>): Run<T> | null {
   ...
   */
  private inOrderTraversal(node: RedBlackNode<T> | null, result: Run<T>[]): void {
    if (node !== null) {
   ...
   */
  private validateNode(node: RedBlackNode<T> | null): number {
    if (node === null) {
   ...
   */
  private calculateMaxEndValue(node: RedBlackNode<T>): number {
   ...
   */
  private nodeToString(node: RedBlackNode<T> | null, prefix: string, isLast: boolean): string {
    if (node === null) {
      return '';
    }
    ...
    let result = prefix;
    result += isLast ? '└── ' : '├── ';
    result += `[${node.interval.start}-${node.interval.end}] "${node.interval.data}" `;
    result += `(${node.color}, max: ${node.maxEndValue})\n`;

    const children: (RedBlackNode<T> | null)[] = [];
   ...
   */
  private getNodeHeight(node: RedBlackNode<T> | null): number {
    if (node === null) {
      return 0;
    }
